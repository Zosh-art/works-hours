...
function buildHolidayWindows(){return JEWISH_HOLIDAYS_RAW.map(h=>{const[ey,em,ed]=h.eve,[dy,dm,dd]=h.endDay,s=getSunsetIL(ey,em,ed);const start=new Date(ey,em-1,ed,s.h,s.m,0,0).getTime();const trueEnd=new Date(dy,dm-1,dd);trueEnd.setDate(trueEnd.getDate()+1);trueEnd.setHours(0,0,0,0);const end=new Date(dy,dm-1,dd);end.setDate(end.getDate()+1);end.setHours(6,0,0,0);return{start,end:end.getTime(),trueEnd:trueEnd.getTime(),name:h.name};});}
...
function getShabbatWindowFor(fridayDate){const s=getSunsetIL(fridayDate.getFullYear(),fridayDate.getMonth()+1,fridayDate.getDate());const start=new Date(fridayDate);start.setHours(s.h,s.m,0,0);const trueEnd=new Date(fridayDate);trueEnd.setDate(fridayDate.getDate()+2);trueEnd.setHours(0,0,0,0);const end=new Date(fridayDate);end.setDate(fridayDate.getDate()+2);end.setHours(6,0,0,0);return{start:start.getTime(),end:end.getTime(),trueEnd:trueEnd.getTime()};}
...
function splitSession(startTs,endTs,isContinuation=false){
  const totalMs=endTs-startTs;
  const candidates=[
    getShabbatWindowFor(getPrecedingFriday(startTs)),
    getShabbatWindowFor(getUpcomingFriday(startTs)),
    ...HOLIDAY_WINDOWS,
  ];
  const seen=new Set();
  let premiumMs=0;
  for(const w of candidates){
    if(seen.has(w.start))continue; // דה-דופליקציה: אם שני המועמדים (שישי קודם/הבא) הם בפועל אותו שבוע
    seen.add(w.start);
    if(!isContinuation&&startTs>=w.trueEnd)continue; // משמרת "טרייה" (לא המשך של אתמול) שהתחילה אחרי שהשבת/החג נגמרו בפועל — לא זכאית מהחלון הזה
    const os=Math.max(startTs,w.start),oe=Math.min(endTs,w.end);
    if(oe>os)premiumMs+=(oe-os);
  }
  premiumMs=Math.min(premiumMs,totalMs);
  return{regularMs:totalMs-premiumMs,premiumMs};
}
...
function dayKeyToDate(key){const[y,m,d]=key.split("-").map(Number);return new Date(y,m,d);}
function isMidnightContinuation(data,dayKey,sessionStart){
  const dayDate=dayKeyToDate(dayKey);
  if(!isExactMidnight(sessionStart,dayDate))return false;
  const prevDate=new Date(dayDate);prevDate.setDate(prevDate.getDate()-1);
  const prevEntry=data[getDayKey(prevDate)];
  const prevRaw=prevEntry?.sessions||[];
  const prevLast=prevRaw[prevRaw.length-1];
  return!!(prevLast&&endsAtDayBoundary(prevLast.end,prevDate));
}
...
function calcEarnings(sessions,activeStart=null,hourlyRate=52.19,data=null,dayKey=null){
  let regularMs=0,premiumMs=0;
  const all=[...(sessions||[])];
  if(activeStart)all.push({start:activeStart,end:Date.now()});
  all.forEach((s,i)=>{
    const isCont=i===0&&data&&dayKey&&isMidnightContinuation(data,dayKey,s.start);
    const sp=splitSession(s.start,s.end,isCont);
    regularMs+=sp.regularMs;premiumMs+=sp.premiumMs;
  });
    ...
    <div style={{position:"fixed",inset:0,background:T.modalOverlay,display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}}>
      <div style={{background:T.surface,borderRadius:20,padding:24,width:"100%",maxWidth:380,maxHeight:"80vh",overflowY:"auto",border:`1px solid ${T.border}`}}>
        ...
        <div style={{display:"flex",gap:6,marginBottom:16}}>
          {[{key:"vacation",lbl:"🏖️ חופשה"},{key:"sick",lbl:"🤒 מחלה"}].map(opt=>{
            const active=dayType===opt.key;
            return(
              <button key={opt.key} onClick={()=>onSetDayType(active?null:opt.key)} style={{flex:1,padding:"8px 10px",borderRadius:10,border:active?"none":`1px solid ${T.border}`,cursor:"pointer",fontSize:12,fontWeight:700,background:active?T.accent:T.surface2,color:active?"#fff":T.textSub}}>{opt.lbl}</button>
            );
          })}
        </div>
...
export default function WorkHoursTracker(){
  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(ref,{includeMetadataChanges:true},async(snap)=>{
      ...
      if(snap.exists()){
        ...
        setDocLoaded(true); // תמיד משחררים את המסך, גם כשעדיין מחכים לאישור מהשרת שהמסמך לא קיים
      }
    },()=>setDocLoaded(true));
    return unsub;
  },[user]);
  ...
  const todayEarnings=useMemo(()=>calcEarnings(todayData.sessions,todayData.active,hourlyRate,data,todayKey),[todayData,now,hourlyRate,data,todayKey]);
  const monthToDateHours=useMemo(()=>{const ty=now.getFullYear(),tm=now.getMonth(),td=now.getDate();let totalMs=0,total=0;for(let i=1;i<=td;i++){const d=new Date(ty,tm,i),key=getDayKey(d),entry=data[key];if(!entry)continue;const earn=calcEarnings(entry.sessions,i===td?entry.active:null,hourlyRate,data,key);totalMs+=earn.totalMs;total+=earn.total;}return{totalMs,total};},[data,now.getFullYear(),now.getMonth(),now.getDate(),hourlyRate]);
  ...
  function handleSetDayType(dateKey,type){
    setDayTypes((prev)=>{
      if(!type){const next={...prev};delete next[dateKey];return next;}
  ...
  return (
    <div style={{minHeight:"100vh",background:`radial-gradient(circle at 1px 1px, rgba(42,38,32,0.05) 1px, transparent 0) 0 0/16px 16px, ${T.bg}`,color:T.text,fontFamily:"'Rubik','Segoe UI',system-ui,sans-serif",direction:"rtl",display:"flex",flexDirection:"column",alignItems:"center",paddingBottom:80}}>
      ...
      {view==="clock"&&(()=>{
        ...
        const hoursActive=isCheckedIn&&trueActiveStart?(now.getTime()-trueActiveStart)/3600000:0;
        return (
        <div style={{width:"100%",maxWidth:480,padding:"18px 20px",display:"flex",flexDirection:"column",alignItems:"center",gap:18}}>
          {hoursActive>10&&(
            <div style={{width:"100%",background:T.accentLight,border:`1px solid ${T.accent}`,borderRadius:12,padding:"10px 14px",display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:18}}>⏰</span>
              <span style={{fontSize:12,color:T.text,fontWeight:600}}>את/ה מחובר/ת כבר {formatTime(hoursActive*3600000)} שעות — שכחת לצאת?</span>
            </div>
          )}
          ...
          <div style={{background:T.surface,borderRadius:16,border:`1px solid ${T.border}`,padding:"14px 10px",width:"100%",display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:6}}>
            ...
            {todayEarnings.premiumMs>0&&todayEarnings.regularMs>0&&(
              <div style={{gridColumn:"1 / -1",marginTop:6}}>
                <div style={{display:"flex",height:4,borderRadius:3,overflow:"hidden"}}>
                  <div style={{flex:todayEarnings.premiumMs,background:T.violet}}/>
                  <div style={{flex:todayEarnings.regularMs,background:T.accent}}/>
                </div>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:T.textFaint,marginTop:3}}>
                  <span>{formatTime(todayEarnings.premiumMs)} ✦ שבת/חג</span>
                  <span>{formatTime(todayEarnings.regularMs)} רגיל</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div style={{width:"100%",maxWidth:480,padding:"16px 20px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            {days.map(({date,earnings,entry})=>{
              ...
              const isMixedDay=earnings.premiumMs>0&&earnings.regularMs>0;
                  ...
                  <div onClick={()=>earnings.totalMs>0?setExpandedDay(isExp?null:getDayKey(date)):setManualEntry({date})} style={{background:isToday?T.todayBg:T.surface,borderRadius:12,padding:"11px 14px",border:`1px solid ${isToday?T.todayBorder:T.border}`,position:"relative",overflow:"hidden",cursor:"pointer"}}>
                    ...
                    {isMixedDay&&(
                      <div style={{position:"relative",marginTop:8}}>
                        <div style={{display:"flex",height:4,borderRadius:3,overflow:"hidden"}}>
                          <div style={{flex:earnings.premiumMs,background:T.violet}}/>
                          <div style={{flex:earnings.regularMs,background:T.accent}}/>
                        </div>
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:T.textFaint,marginTop:3}}>
                          <span>{formatTime(earnings.premiumMs)} ✦ שבת/חג</span>
                          <span>{formatTime(earnings.regularMs)} רגיל</span>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  {isExp&&entry&&(
                    <div style={{background:T.expandedBg,borderRadius:"0 0 12px 12px",padding:"12px 14px",border:`1px solid ${T.border}`,borderTop:"none",marginTop:-4}}>
                      {[...(entry.sessions||[]),...(entry.active?[{start:entry.active,end:Date.now(),live:true}]:[])].map((s,i)=>{
                        const isCont=i===0&&isMidnightContinuation(data,getDayKey(date),s.start);
                        const sp=splitSession(s.start,s.end,isCont);
                        const earn=(sp.regularMs/3600000)*hourlyRate+(sp.premiumMs/3600000)*hourlyRate*PREMIUM_RATE;
                        const sessionMixed=sp.premiumMs>0&&sp.regularMs>0;
                        return (<div key={i} style={{padding:"7px 0",borderBottom:`1px solid ${T.border}`,fontSize:12}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            <span style={{color:T.textSub}}>{new Date(s.start).toLocaleTimeString("he-IL",{hour:"2-digit",minute:"2-digit"})} ← {s.live?"עכשיו":new Date(s.end).toLocaleTimeString("he-IL",{hour:"2-digit",minute:"2-digit"})}{sp.premiumMs>0&&!sessionMixed&&<span style={{color:T.violet,marginRight:4}}> ✦</span>}</span>
                            <span style={{color:T.gold,fontWeight:600}}>{formatMoney(earn)}</span>
                          </div>
                          {sessionMixed&&(
                            <div style={{marginTop:5}}>
                              <div style={{display:"flex",height:4,borderRadius:3,overflow:"hidden"}}>
                                <div style={{flex:sp.premiumMs,background:T.violet}}/>
                                <div style={{flex:sp.regularMs,background:T.accent}}/>
                              </div>
                              <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:T.textFaint,marginTop:3}}>
                                <span>{formatTime(sp.premiumMs)} ✦ שבת/חג</span>
                                <span>{formatTime(sp.regularMs)} רגיל</span>
                              </div>
                            </div>
                          )}
                        </div>);
