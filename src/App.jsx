...
function numToGematria(n) { const h = Math.floor(n / 100), t = Math.floor((n % 100) / 10), u = n % 10; let s = (HEB_HUNDREDS[h] || "") + (HEB_TENS[t] || "") + (HEB_UNITS[u] || ""); if (!s)
    return ""; return s.length === 1 ? s + "׳" : s.slice(0, -1) + "״" + s.slice(-1); }
function toHebrewDate(date) { try {
...
// ── Parasha ───────────────────────────────────────────────────────────────────
const parashaCache = {};
async function fetchParasha(saturdayDate) { const key = `${saturdayDate.getFullYear()}-${String(saturdayDate.getMonth() + 1).padStart(2, "0")}-${String(saturdayDate.getDate()).padStart(2, "0")}`; if (parashaCache[key])
    return parashaCache[key]; try {
    const url = `https://www.hebcal.com/hebcal?v=1&cfg=json&maj=off&min=off&mod=off&nx=off&year=${saturdayDate.getFullYear()}&month=${saturdayDate.getMonth() + 1}&ss=off&mf=off&c=off&s=on&i=on&lg=he&leyning=off`;
    ...
    if (item?.hebrew) {
        parashaCache[key] = item.hebrew;
        return item.hebrew;
    }
    return "";
}
catch (err) {
    console.error("fetchParasha failed for", key, err);
    return "";
} }
...
function getSpecialShabbat(date) { const { monthStr, dayNum } = toHebrewDate(date); for (const r of SPECIAL_SHABBAT_RULES) {
    if (r.months.includes(monthStr) && dayNum >= r.range[0] && dayNum <= r.range[1])
        return r.name;
} return ""; }
function formatParashaLabel(name, special) { if (!name)
    return ""; return special ? `${name} (${special})` : name; }
function getSaturdayOf(date) { const d = new Date(date), day = d.getDay(); if (day === 6)
    return d; if (day === 5) {
...
function getShabbatWindow(dateTs) { const fd = new Date(dateTs); fd.setHours(0, 0, 0, 0); while (fd.getDay() !== 5)
    fd.setDate(fd.getDate() + (fd.getDay() < 5 ? 5 - fd.getDay() : 7 - (fd.getDay() - 5))); const s = getSunsetIL(fd.getFullYear(), fd.getMonth() + 1, fd.getDate()); const start = new Date(fd); start.setHours(s.h, s.m, 0, 0); const end = new Date(fd); end.setDate(fd.getDate() + 2); end.setHours(6, 0, 0, 0); return { start: start.getTime(), end: end.getTime() }; }
function isInPremiumWindow(ts) { const sw = getShabbatWindow(ts); if (ts >= sw.start && ts < sw.end)
    return true; for (const hw of HOLIDAY_WINDOWS)
    if (ts >= hw.start && ts < hw.end)
...
function getNextPremiumStart(startTs, endTs) { const c = []; const sw = getShabbatWindow(startTs); if (sw.start > startTs && sw.start < endTs)
    ...
    if (es > startTs && es < endTs)
        c.push(es); return c.length ? Math.min(...c) : null; }
function splitSession(startTs, endTs) { const totalMs = endTs - startTs; if (isInPremiumWindow(startTs))
    return { regularMs: 0, premiumMs: totalMs }; const ps = getNextPremiumStart(startTs, endTs); if (ps !== null)
    return { regularMs: ps - startTs, premiumMs: endTs - ps }; return { regularMs: totalMs, premiumMs: 0 }; }
function getHolidayName(ts) { for (const hw of HOLIDAY_WINDOWS)
    if (ts >= hw.start && ts < hw.end)
        return hw.name; return null; }
...
function getDayHolidayInfo(date) {
    ...
    if (asEve)
        return { type: "eve", label: asEve.eveName };
    ...
    if (asChanukah)
        return { type: "holiday", label: "חנוכה" };
    const asMinor = MINOR_JEWISH_DAYS.find(md => md.date[0] === y && md.date[1] === m && md.date[2] === d);
    if (asMinor)
        return { type: asMinor.name === "פורים" ? "holiday" : "fast", label: asMinor.name };
    return null;
}
// ── Helpers ───────────────────────────────────────────────────────────────────
function formatTime(ms) { if (!ms || ms <= 0)
    return "0:00"; return `${Math.floor(ms / 3600000)}:${String(Math.floor((ms % 3600000) / 60000)).padStart(2, "0")}`; }
...
function carryOverMidnight(prevData, todayKeyNow) {
    ...
    for (const key of Object.keys(prevData)) {
        ...
        if (!entry?.active || key === todayKeyNow)
            continue;
        ...
        while (cursorKey !== todayKeyNow) {
            ...
            cursorYear = nd.getFullYear();
            cursorMonth = nd.getMonth();
            cursorDay = nd.getDate();
...
function calcEarnings(sessions, activeStart = null, hourlyRate = 52.19) { let regularMs = 0, premiumMs = 0; const all = [...(sessions || [])]; if (activeStart)
    all.push({ start: activeStart, end: Date.now() }); for (const s of all) {
...
function JournalDayModal({ date, sessions, notes, parasha, specialShabbat, onAddNote, onDeleteNote, onSetShiftOverride, onMergeSessions, onClose, T }) {
    ...
    return (<div style={{ position: "fixed", inset: 0, background: T.modalOverlay, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
      <div style={{ background: T.surface, borderRadius: 20, padding: 24, width: "100%", maxWidth: 380, maxHeight: "80vh", overflowY: "auto", border: `1px solid ${T.border}` }}>
        ...
        {(notes || []).length > 0 ? (notes.map(n => (<div key={n.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: T.surface2, borderRadius: 10, marginBottom: 6, fontSize: 13 }}>
          ...
          </div>))) : (<div style={{ fontSize: 12, color: T.textFaint, marginBottom: 8 }}>אין הערות עדיין</div>)}
...
export default function WorkHoursTracker() {
    ...
    const [journalNotes, setJournalNotes] = useState(() => { try {
    ...
    useEffect(() => { try {
        localStorage.setItem(WAGE_KEY, String(hourlyRate));
    }
    catch { } }, [hourlyRate]);
    useEffect(() => { try {
        localStorage.setItem(JOURNAL_KEY, JSON.stringify(journalNotes));
    }
    catch { } }, [journalNotes]);
    ...
    const monthToDateHours = useMemo(() => { const ty = now.getFullYear(), tm = now.getMonth(), td = now.getDate(); let totalMs = 0, total = 0; for (let i = 1; i <= td; i++) {
        const d = new Date(ty, tm, i), key = getDayKey(d), entry = data[key];
        if (!entry)
            continue;
        const earn = calcEarnings(entry.sessions, i === td ? entry.active : null, hourlyRate);
    ...
    useEffect(() => { if (!isFriOrSat) {
        setTodayParasha("");
        return;
    } const sat = getSaturdayOf(now); if (sat)
        fetchParasha(sat).then(p => setTodayParasha(p)); }, [todayKey]);
    const todaySpecialShabbat = useMemo(() => { if (!isFriOrSat)
        return ""; const sat = getSaturdayOf(now); return sat ? getSpecialShabbat(sat) : ""; }, [todayKey]);
    useEffect(() => { const WEATHER_ICONS = { 0: "☀️", 1: "🌤️", 2: "⛅", 3: "☁️", 45: "🌫️", 51: "🌦️", 61: "🌧️", 71: "🌨️", 80: "🌧️", 95: "⛈️" }; async function load() { try {
        ...
        const json = await res.json();
        const d = json.daily;
        setWeather(d.time.slice(1, 3).map((dt, i) => ({ label: ["מחר", "מחרתיים"][i], high: Math.round(d.temperature_2m_max[i + 1]), low: Math.round(d.temperature_2m_min[i + 1]), icon: WEATHER_ICONS[d.weathercode[i + 1]] || "🌡️" })));
    }
    catch {
        setWeather([]);
    } } load(); }, [todayKey]);
    ...
    useEffect(() => { const result = {}; const jobs = []; for (let i = 1; i <= daysInMonth; i++) {
        const d = new Date(year, month, i);
        if (d.getDay() !== 6)
            continue;
        const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
        jobs.push(fetchParasha(d).then(p => { if (p)
            result[key] = p; }));
    ...
    return (<div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "'Segoe UI',system-ui,sans-serif", direction: "rtl", display: "flex", flexDirection: "column", alignItems: "center", paddingBottom: 80 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {days.map(({ date, earnings, entry }) => {
                ...
                return (<div key={date.getDate()}>
                  ...
                  {isExp && entry && (<div style={{ background: T.expandedBg, borderRadius: "0 0 12px 12px", padding: "12px 14px", border: `1px solid ${T.border}`, borderTop: "none", marginTop: -4 }}>
      ...
      {/* ── Journal view ── */}
      {view === "journal" && (<div style={{ width: "100%", maxWidth: 480, padding: "16px 20px" }}>
